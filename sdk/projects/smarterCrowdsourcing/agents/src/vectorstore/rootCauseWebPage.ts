...
export class RootCauseWebPageVectorStore extends PolicySynthSimpleAgentBase {
  ...
  async addSchema() {
    let classObj;
    try {
      const data = await fs.readFile("./schemas/rootCauseWebPage.json", "utf8");
  ...
  async postWebPage(webPageAnalysis: PSRootCauseRawWebPageData) {
    return new Promise((resolve, reject) => {
      RootCauseWebPageVectorStore.client.data
        ...
        .do()
        .then((res) => {
          this.logger.info(`Weaviate: Have saved root cause web page ${(webPageAnalysis as any).url}`);
          resolve(res);
        })
        .catch((err) => {
          reject(err);
  ...
  async updateWebPage(id: string, webPageAnalysis: PSRootCauseRawWebPageData) {
    return new Promise((resolve, reject) => {
      RootCauseWebPageVectorStore.client.data
        .updater()
        .withId(id)
        ...
        .do()
        .then((res) => {
          this.logger.info(`Weaviate: Have updated web page ${(webPageAnalysis as any).url}`);
          resolve(res);
        })
        .catch((err) => {
          reject(err);
  ...
  async updateWebRootCause(id: string, rootCauseType: string, rootCauses: string[], quiet = false) {
    const props = {} as any;
    ...
    return new Promise((resolve, reject) => {
      RootCauseWebPageVectorStore.client.data
        ...
        .do()
        .then((res) => {
          if (!quiet) this.logger.info(`Weaviate: Have updated web solutions for ${id}`);
          resolve(res);
        })
        .catch((err) => {
  ...
  async saveWebPageMetadata(id: string, metadata: PSWebPageMetadata, quiet = false) {
    const props = {} as any;
    return new Promise((resolve, reject) => {
      RootCauseWebPageVectorStore.client.data
        .merger()
  ...
  async updateRefinedAnalysis(id: string, refinedRootCause: PSRefinedRootCause, quiet = false) {
    const totalScore =
      (refinedRootCause.rootCauseRelevanceToProblemStatementScore ?? 0) +
    ...
    return new Promise((resolve, reject) => {
      RootCauseWebPageVectorStore.client.data
        ...
        .withId(id)
        .withClassName("RootCauseWebPage")
        .withProperties(refinedRootCause as any)
        .do()
        .then((res) => {
          if (!quiet) this.logger.info(`Weaviate: Have updated refined root cause type for ${id}`);
          resolve(res);
        })
        .catch((err) => {
          this.logger.error(err.stack || err);
  ...
  async updateScores(id: string, scores: PSRootCauseRating, quiet = false) {
    return new Promise((resolve, reject) => {
      RootCauseWebPageVectorStore.client.data
        .merger()
        .withId(id)
        ...
        .do()
        .then((res) => {
          if (!quiet) this.logger.info(`Weaviate: Have updated scores for ${id}`);
          resolve(res);
        })
        .catch((err) => {
          this.logger.error(err.stack || err);
  ...
  async getWebPage(id: string): Promise<PSRootCauseRawWebPageData> {
    return new Promise((resolve, reject) => {
      RootCauseWebPageVectorStore.client.data
        .getterById()
        .withId(id)
  ...
  async webPageExist(groupId: number, url: string, searchType: PSRootCauseWebPageTypes): Promise<Boolean> {
    //TODO: Fix any here
    const where: any[] = [];
  ...
  async searchWebPages(
    ...
    searchType: PSRootCauseWebPageTypes | undefined,
  ): Promise<PSRootCauseWebPageGraphQlResults> {
    //TODO: Fix any here
    const where: any[] = [];
    ...
    try {
      results = await RootCauseWebPageVectorStore.client.graphql
        ...
        .withLimit(25) //TODO: Get from config
        .withWhere({
          operator: "And",
          operands: where,
        })
    ...
    return results as PSRootCauseWebPageGraphQlResults;
  }
}
