...
export class PsRagRouter extends BaseIngestionAgent {
  systemMessage = (schema: string, about: string) =>
    new SystemMessage(`You are an expert user question analyzer for a RAG based chatbot. We will use the information to decide what documents to retrieve for the user.
...
`);
  ...
  userMessage = (question: string) =>
    new HumanMessage(`<QUESTION_FROM_USER>${question}</QUESTION_FROM_USER>
