...
`);
  ...
  splitIndexSystemMessage =
    new SystemMessage(`You are an expert document splitter.
...
`);
  ...
  async chunkDocument(
    ...
    if (this.getEstimateTokenLength(data) < this.minChunkTokenLength) {
      ...
      const chunkingStrings =
        chunkIdentifiersResponse.oneLineTextIndexesForSplittingDocument;
      ...
      if (chunkingStrings) {
        chunkingStrings.forEach((chunkStr: string, index) => {
