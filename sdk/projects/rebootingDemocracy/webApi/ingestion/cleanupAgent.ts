...
`);
  ...
  userMessage = (data: string) =>
    new HumanMessage(`Document to cleanup and output in full:
${data}
`);
  ...
  async clean(data: string) {
    ...
    for (const part of splitPartsForCleanup) {
      const cleanedPart = (await this.callLLM(
        "ingestion-agent",
        IEngineConstants.ingestionModel,
        this.getFirstMessages(this.systemMessage, this.userMessage(part)),
        false
      )) as string;
