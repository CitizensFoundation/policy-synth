...
export class RebootingDemocracyChatBotApp extends PolicySynthWebApp {
    ...
    saveChatToLocalStorage() {
        const chatBotElement = this.$$('rebooting-democracy-chat-bot');
        if (chatBotElement &&
    ...
    async getChatLogFromServer() {
        const chatBotElemement = this.$$('rebooting-democracy-chat-bot');
        if (chatBotElemement &&
            chatBotElemement.chatLog.length == 0 &&
            this.serverMemoryId) {
            const { chatLog, totalCosts } = await this.serverApi.getChatLogFromServer(this.serverMemoryId);
            if (totalCosts) {
    ...
    updateRunningTime() {
        if (this.startTime) {
            ...
            // Update every second
            this.runningTimeInterval = setTimeout(() => this.updateRunningTime(), 1000);
        }
    }
    startTimer() {
