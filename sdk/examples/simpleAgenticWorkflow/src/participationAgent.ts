...
export class ParticipationDataAnalysisAgent extends PolicySynthAgent {
  ...
  private static readonly PARTICIPATION_DATA_AGENT_CLASS_BASE_ID =
    "7bee275a-077b-40da-8f33-a4d8c647edc3";

  /**
   * Increment this whenever you introduce a new version of your agent’s logic.
   */
  private static readonly PARTICIPATION_DATA_AGENT_CLASS_VERSION = 1;
   ...
   */
  public static getAgentClass(): PsAgentClassCreationAttributes {
    return {
      ...
      configuration: {
        ...
        description:
          "An agent for analyzing participation data from Yrpri, applying theme & sentiment analysis, and exporting results.",
        queueName: "PARTICIPATION_DATA_ANALYSIS",
        imageUrl:
          "https://assets.evoly.ai/ypGenAi/domain/1/4170a8c1-0d30-4d8e-b9a1-0b782c557cb1.png",
   ...
   */
  public static getExtraConfigurationQuestions(): YpStructuredQuestionData[] {
    return [
      {
   ...
   */
  public static getDefaultStructuredQuestions(): YpStructuredQuestionData[] {
    return [
      {
        type: "textFieldLong",
        text: "Title",
  ...
  private getAnswerValue(
    structuredAnswers: YpStructuredAnswer[],
    uniqueId: string
  ): any {
  ...
  translateStructuredAnswersToItem(
    structuredAnswers: YpStructuredAnswer[],
    itemName: string
  ): ParticipationData {

    const item: ParticipationData = {
  ...
  async process() {
    ...
    const yrpriConnectorIn = PsConnectorFactory.getConnector(
      this.agent,
      this.memory,
      PsConnectorClassTypes.IdeasCollaboration,
      true
    ) as PsBaseIdeasCollaborationConnector;
    ...
    const analyzeAgent = new AnalyzeItemThemesAndSentimentAgent(
      this.agent,
      this.memory,
      0,
      40
    ...
    const reportAgent = new GenerateFullReportAgent(
      this.agent,
      this.memory,
      40,
      60
    ...
    const sheetsAgent = new SheetsExportParticipationDataAgent(
      this.agent,
      this.memory,
      60,
      80,
      "Sheet1" // The sheet name or tab name
    );
    await sheetsAgent.process();

    // 5) Export the summary report to Google Docs
    const docsAgent = new DocsExportParticipationDataAgent(
      this.agent,
      this.memory,
      80,
      100
