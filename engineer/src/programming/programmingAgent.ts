...
export class PsEngineerProgrammingAgent extends PolicySynthAgentBase {
  ...
  async implementCodingPlan(fileName: string, codingPlan: string) {
    ...
    if (file) {
      ...
      fs.copyFileSync(
        path.join(this.memory.workspaceFolder, fileName),
        path.join(this.memory.workspaceFolder, backupFileName)
      );
      file.saveSync();
  ...
  async getCodingPlan(fileName: string) {
    let planReady = false;
    let planRetries = 0;
    let reviewLog = "";
