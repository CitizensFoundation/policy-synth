...
`);
    validationUserMessage = (original, cleaned) => this.createHumanMessage(`<ORIGINAL_TEXT>${original}</ORIGINAL_TEXT>

<CLEANED_TEXT>${cleaned}</CLEANED_TEXT>

Think step by step and output your analysis here:
`);
    systemMessage = this.createSystemMessage(`You are an expert document cleaner. Your job is to help cleanup documents coming from various sources. PDFs, etc.
...
`);
    userMessage = (data, validationTextResults) => this.createHumanMessage(`${validationTextResults
...
${data}
`);
    referencesCheckSystemMessage = this.createSystemMessage(`Please analyze this document if it contains paragraphs, sentences or only a list of references or urls or references with urls.
...
`);
    referencesCheckUserMessage = (data) => this.createHumanMessage(`Document to analyze:
${data}
...
`);
    async clean(data) {
        ...
        const cleanPart = async (part, index, total) => {
            ...
            while (!validated && retryCount < this.maxCleanupRetries) {
                const referenceAnalysis = (await this.callLLM("ingestion-agent", this.getFirstMessages(this.systemMessage, this.userMessage(part, validationTextResults)), false));
                if (referenceAnalysis.indexOf("ONLY_REFERENCES_OR_URLS") > -1) {
                    ...
                    cleanedPart = (await this.callLLM("ingestion-agent", this.getFirstMessages(this.systemMessage, this.userMessage(part, validationTextResults)), false));
    ...
    async validateCleanedPart(original, cleaned) {
        ...
        const validations = await Promise.all([
            this.callLLM("ingestion-agent", this.getFirstMessages(this.completionValidationSystemMessage, this.validationUserMessage(original, cleaned)), false),
            this.callLLM("ingestion-agent", this.getFirstMessages(this.correctnessValidationSystemMessage, this.validationUserMessage(original, cleaned)), false),
            this.callLLM("ingestion-agent", this.getFirstMessages(this.hallucinationValidationSystemMessage, this.validationUserMessage(original, cleaned)), false),
        ]);
