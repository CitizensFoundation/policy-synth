...
export class CreateSolutionsVectorStoreProcessor extends SolutionsSmarterCrowdsourcingAgent {
    ...
    async renderRefinePrompt(results, generalTextContext, scientificTextContext, openDataTextContext, newsTextContext, subProblemIndex, alreadyCreatedSolutions = undefined) {
        const messages = [
            this.createSystemMessage(`As an expert, your task is to refine innovative solution components proposed for problems and associated sub-problems.
    ...
    renderCreateSystemMessage() {
        return this.createSystemMessage(`As an expert, you are tasked with creating innovative solution components for sub problems, considering the affected entities.
    ...
    async createSolutions(subProblemIndex, generalTextContext, scientificTextContext, openDataTextContext, newsTextContext, alreadyCreatedSolutions = undefined, stageName = "create-seed-solutions") {
        if (DISABLE_LLM_FOR_DEBUG) {
            ...
            let results = await this.callModel(PsAiModelType.Text, await this.renderCreatePrompt(generalTextContext, scientificTextContext, openDataTextContext, newsTextContext, subProblemIndex, alreadyCreatedSolutions), true, false, 860);
            if (this.createSolutionsRefineEnabled) {
                this.logger.info(`Calling LLM refine for sub problem ${subProblemIndex}`);
                results = await this.callModel(PsAiModelType.Text, await this.renderRefinePrompt(results, generalTextContext, scientificTextContext, openDataTextContext, newsTextContext, subProblemIndex, alreadyCreatedSolutions), true, false, 860);
            }
            return results;
        }
    }
    randomSearchQueryIndex(searchQueries, type) {
        const randomIndex = Math.min(Math.floor(Math.random() * (this.maxTopSearchQueriesForSolutionCreation + 1)), searchQueries[type].length - 1);
        if (Math.random() < this.createSolutionsNotUsingTopSearchQueriesChance) {
            ...
            const randomTop = Math.min(Math.floor(Math.random() * (this.maxTopQueriesToSearchPerType + 1)), searchQueries[type].length - 1);
    ...
    getRandomSearchQueryForType(type, problemStatementQueries, subProblemQueries, otherSubProblemQueries, randomEntitySearchQueries) {
        ...
        let selectedQuery;
        const mainProblemChance = this.createSolutionsSearchQueriesUseMainProblemSearchQueriesChance;
    ...
    getSearchQueries(subProblemIndex) {
        ...
        for (let i = 0; i < Math.min(this.memory.subProblems.length, this.maxSubProblems); i++) {
        ...
        const chosenEntities = entities.slice(0, this.memory.groupId === 1 ? 3 : this.maxTopEntitiesToSearch);
    ...
    renderRawSearchResults(rawSearchResults) {
        const results = this.getRandomItemFromArray(rawSearchResults.data.Get.WebPage, this.useRandomTopFromVectorSearchResultsLimits);
    ...
    async getSearchQueryTextContext(subProblemIndex, searchQuery, type, alreadyCreatedSolutions = undefined) {
        ...
        const tokenCountData = await this.getTokensFromMessages(this.renderCreateForTestTokens(subProblemIndex, alreadyCreatedSolutions));
        const currentTokens = tokenCountData;
        const tokensLeft = this.tokenInLimit - (currentTokens + this.maxModelTokensOut);
