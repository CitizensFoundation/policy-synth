...
export class PsSubProblemsReportGenerator extends BaseSmarterCrowdsourcingAgent {
    summaryCount;
    constructor(memoryData) {
        super(undefined, memoryData, 1, 1);
        this.summaryCount = 0;
    }
    async renderPairwiseChoicesPrompt(items, previousSummary) {
        const messages = [
            this.createSystemMessage(`You are an expert in creating a list of choices to vote in on a pairwise ranking project.
        ...
        ${previousSummary
                ...
                : ""}`),
            this.createHumanMessage(`${this.renderProblemStatement()}
    ...
    async renderSummaryPrompt50(items, previousSummary) {
        const messages = [
            this.createSystemMessage(`You are an expert in summarizing and reporting on root causes.
 ...
 ${previousSummary
    ...
    async renderSummaryPromptNovel(items, previousSummary) {
        const messages = [
            this.createSystemMessage(`You are an expert in summarizing and reporting on novel, unexpected and outying root causes of problems.
 ...
 ${previousSummary
    ...
    async summarizeItems(items, previousSummary) {
        try {
            ...
            const summary = await this.callModel(PsAiModelType.Text, await this.renderSummaryPrompt(items, previousSummary), false);
            this.logger.debug(`Summary coming out of LLM ${summary}`);
            this.summaryCount++;
            const fileName = `/tmp/subProblemSummary${this.memory.agentId}Response${this.summaryCount}.md`;
