    ...
    async setupAgentQueue() {
        if (this.agentQueueName) {
            ...
            const worker = new Worker(this.agentQueueName, async (job) => {
                try {
                    console.log(`Processing job ${job.id} on worker for queue ${this.agentQueueName}`);
                    const data = job.data;
            ...
            worker.on("completed", (job) => {
                this.logger.info(`Job ${job.id} has completed on worker for queue ${this.agentQueueName}`);
            });
            worker.on("failed", (job, err) => {
                this.logger.error(`Job ${job?.id || "unknown"} failed on worker for queue ${this.agentQueueName}`, err);
            });
            worker.on("error", (err) => {
                this.logger.error(`An error occurred in worker for queue ${this.agentQueueName}`, err);
            });
            worker.on("active", (job) => {
                this.logger.info(`Job ${job.id} started on worker for queue ${this.agentQueueName}`);
            });
            worker.on("stalled", (jobId) => {
                this.logger.warn(`Job ${jobId} has been stalled on worker for queue ${this.agentQueueName}`);
            });
