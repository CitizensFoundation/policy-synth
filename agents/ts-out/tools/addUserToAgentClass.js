...
async function addUserToAgentClasses(agentClassBaseId, userId, role) {
    try {
        ...
        for (const agentClass of agentClasses) {
            ...
            if (!hasAccess) {
                ...
                if (role === "user") {
                    ...
                    console.log(`User ${userId} added as user to agent class ${agentClass.id}`);
                }
                else if (role === "admin") {
                    await agentClass.addAdmin(user);
                    console.log(`User ${userId} added as admin to agent class ${agentClass.id}`);
                }
            }
            else {
                console.log(`User ${userId} already has ${role} access to agent class ${agentClass.id}`);
            }
        }
    }
    catch (error) {
...
if (args.length !== 3) {
    console.error("Usage: ts-node addUserToAgentClasses.ts <agentClassBaseId> <userId> <role>");
    process.exit(1);
}
...
// Run the function
addUserToAgentClasses(agentClassBaseId, Number(userId), role);
