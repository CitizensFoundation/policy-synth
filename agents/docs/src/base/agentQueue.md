...
The `PolicySynthAgentQueue` class is an abstract class that extends the `PolicySynthAgent` class. It is designed to manage and process a queue of agents using the BullMQ library. This class handles the setup of the agent queue, processing of agents, and updating their statuses.

## Properties

| Name          | Type   | Description               |
|---------------|--------|---------------------------|
| startProgress | number | The initial progress value for the agent processing. |
| endProgress   | number | The final progress value for the agent processing.    |
| agent         | PsAgent | The agent being processed.                           |
| memory        | any    | The memory associated with the agent.                 |

## Methods

| Name               | Parameters        | Return Type | Description                 |
|--------------------|-------------------|-------------|-----------------------------|
| constructor        | -                 | -           | Initializes the `PolicySynthAgentQueue` instance. |
| processors         | -                 | Array       | Abstract method to get the list of processors. Each processor is an object containing a processor class and its weight. |
| processAllAgents   | -                 | Promise<void> | Processes all agents in the queue sequentially. |
| agentQueueName     | -                 | string      | Abstract method to get the name of the agent queue. |
| setupMemoryIfNeeded| -                 | Promise<void> | Abstract method to set up memory if needed. |
| setupAgentQueue    | -                 | Promise<void> | Sets up the agent queue and initializes the worker to process jobs. |
...
| updateAgentStatus  | status: "running" \| "stopped" \| "paused" | Promise<void> | Updates the status of the agent. |
...
In this example, the `PolicySynthAgentQueue` class is used to manage and process a queue of agents. The class includes methods for setting up the agent queue, processing agents, and updating their statuses. The `setupAgentQueue` method initializes a worker to process jobs from the queue, and the `processAllAgents` method processes all agents sequentially. The `startAgent`, `stopAgent`, and `pauseAgent` methods handle the respective actions for the agent.