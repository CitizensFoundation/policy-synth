...
The `PsAgent` class represents an agent in the system, which is a core component for handling various tasks and operations. This class is built using Sequelize ORM and includes various properties and methods to manage agents, their configurations, and their associations with other models.
...
| Class             | PsAgentClass \| undefined   | The class to which the agent belongs.                                       |
| User              | YpUserData \| undefined     | The user who created the agent.                                             |
| Group             | YpGroupData \| undefined    | The group to which the agent belongs.                                       |
| ExternalApiUsage  | PsExternalApiUsageAttributes[] \| undefined | The external API usage records associated with the agent.                   |
| ModelUsage        | PsModelUsageAttributes[] \| undefined | The model usage records associated with the agent.                          |
| ParentAgent       | PsAgent \| undefined        | The parent agent, if any.                                                   |
| SubAgents         | PsAgent[] \| undefined      | The sub-agents associated with this agent.                                  |
...
console.log(agent.redisMemoryKey); // Output: ps:agent:memory:1:<uuid>
console.log(agent.redisStatusKey); // Output: ps:agent:status:1:<uuid>
```