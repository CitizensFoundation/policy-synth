...
export class GoogleDocsReportAgent extends PolicySynthAgent {
  ...
  async processItem(researchItem: GoldplatingResearchItem): Promise<void> {
    await this.updateRangedProgress(0, "Starting Google Docs report generation");

    const rankedArticles = this.collectAndRankArticles(researchItem);
    const reportContent = this.generateReportContent(researchItem, rankedArticles);

    await this.docsConnector.updateDocument(reportContent);

    await this.updateRangedProgress(100, "Google Docs report generation completed");
  }
  ...
  private generateReportContent(researchItem: GoldplatingResearchItem, rankedArticles: ArticleWithRanking[]): string {
    let content = "";
  ...
  private generateExecutiveSummary(rankedArticles: ArticleWithRanking[]): string {
    ...
    rankedArticles.slice(0, 5).forEach((article, index) => {
      summary += `${index + 1}. ${article.source.charAt(0).toUpperCase() + article.source.slice(1)} Article ${article.number}\n`;
      summary += `   ELO Rating: ${article.eloRating}\n`;
      summary += `   Description: ${article.research?.description || "N/A"}\n\n`;
    });

    return summary;
  }

  private generateDetailedFindings(rankedArticles: ArticleWithRanking[]): string {
    let findings = "";

    rankedArticles.forEach((article, index) => {
      findings += `${index + 1}. ${article.source.charAt(0).toUpperCase() + article.source.slice(1)} Article ${article.number}\n`;
      findings += `ELO Rating: ${article.eloRating}\n`;
      findings += `Text: ${article.text}\n`;
      findings += `Description: ${article.research?.description || "N/A"}\n`;
      findings += `Reason for Gold-Plating: ${article.research?.reasonForGoldPlating || "N/A"}\n`;
      findings += `Recommendation: ${article.research?.recommendation || "N/A"}\n\n`;
