...
export class PsRagRouter extends PolicySynthStandaloneAgent {
    topicTitle;
    topicDescription;
    constructor(memory, topicTitle = "", topicDescription = "") {
        super(memory);
    ...
    systemMessage = (chatHistory) => this.createSystemMessage(`You are an expert user question analyzer for a RAG based chatbot. The chatbot focuses on "${this.topicTitle}". Use the topic description below when rewriting the user's question.
...
`);
    async getRoutingData(userQuestion, chatHistory) {
        const routingInformation = await this.callModel(PsAiModelType.Text, PsAiModelSize.Medium, [this.systemMessage(chatHistory), this.userMessage(userQuestion)]);
