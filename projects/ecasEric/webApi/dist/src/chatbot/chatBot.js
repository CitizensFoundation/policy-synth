...
`;
    mainStreamingUserPrompt = (latestQuestion, questionAnswerContext, countryLinksInfo) => `${countryLinksInfo
...
`;
    searchContext;
    currentTopicId;
    constructor(wsClientId, wsClients, memoryId, topicId) {
    ...
    async loadCountryLinksInfo(countrySlug) {
    ...
    async loadQaPairsForTopic(topicId) {
        if (!topicId) {
    ...
    async streamResponse(prompt, chatHistory) {
        if (!this.geminiModel) {
    ...
    async ecasYeaConversation(chatLog) {
        ...
        const systemInstruction = this.mainStreamingSystemPrompt(topicTitle, topicContext);
        ...
        const history = chatLogWithoutLastUserMessage.map((msg) => ({
            role: msg.sender === "user" || msg.sender === "you" ? "user" : "model",
            parts: [{ text: msg.message }],
        }));
        // Call the new streaming method
        await this.streamResponse(this.mainStreamingUserPrompt(userLastMessage, JSON.stringify(this.searchContext, null, 2), countryLinksInfo), history);
    }
}
