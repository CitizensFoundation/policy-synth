...
export class ValidateJobDescriptionAgent extends PolicySynthAgent {
  ...
  async processJobDescription(jobDescription: JobDescription) {
   ...
   if (degreeAnalysis.educationRequirements && degreeAnalysis.educationRequirements.length > 0) {
    ...
    const includesHigherDegreeRequirement = degreeAnalysis.educationRequirements.some((req: any) => {
...
const similarity = (a: string, b: string): number => {
  ...
  if (mentionsDegree(degreeAnalysis.barriersToNonDegreeApplicants)) {
    ...
    const alternativeLanguageMatches = degreeStatus.alternativeQualifications?.some((qualification: any) => {
