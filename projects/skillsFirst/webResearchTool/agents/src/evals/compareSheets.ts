...
/**
 * Representation of a difference in a field's values across multiple connectors.
 */
...
export class SheetsComparisonAgent extends PolicySynthAgent {
  ...
  static getAgentClass(): PsAgentClassCreationAttributes {
    return {
      ...
      configuration: {
        ...
        description:
          "An agent for comparing job descriptions across all available spreadsheet connectors",
  ...
  async process(): Promise<void> {
    ...
    const allImportedResults =
      await importAgent.importJobDescriptionsFromAllConnectors();
    ...
    for (const result of allImportedResults) {
      ...
      this.logger.info(
        `Imported ${data.length} job descriptions from connector "${connectorName}".`
      );
    }
    ...
    for (const titleCode of allTitleCodes) {
      ...
      const rowDataPerConnector: Record<string, JobDescription> = {};
      for (const connectorName of Object.keys(allImportedData)) {
        const match = allImportedData[connectorName].find(
          (d) => d.titleCode === titleCode
        );
        if (match) {
    ...
    for (let i = 0; i < differences.length; i += chunkSize) {
      ...
      await Promise.all(
        chunk.map(async (diff) => {
          ...
          const originalJD: JobDescription | undefined =
            this.memory.jobDescriptions?.find(
              (jd: JobDescription) => jd.titleCode === diff.titleCode
            );

          if (!originalJD) {
            this.logger.warn(
              `No original job description in memory for titleCode=${diff.titleCode}. Skipping LLM resolution.`
            );
            return;
          }

          try {
            const llmResult = await this.resolveDifferenceWithLLM(
              originalJD,
              diff
            );
            ...
            this.logger.error(
              `Error resolving difference for titleCode=${
                diff.titleCode
              }, field=${diff.field}: ${(err as Error).message}`
            );
          }
        })
      );
    ...
    const differencesWithResolution = differences.filter(
      (d) => d.correctConnectorNames
    ).length;
    this.logger.info(
    ...
    this.logger.info(
      "Final correctness counts by connector & field (including 0/0 fields):"
    );
  ...
  ): Promise<ComparisonDifferenceReturn> {
...
${diff.values
  .map(
    (pair) =>
      `<ValueFromConnector name="${pair.connectorName}">${
        pair.value ?? "(empty)"
      }</ValueFromConnector>`
  )
  .join("\n")}
