...
interface ComparisonDifference {
  titleCode: string;
  field: string;
  values: { sheetName: string; value: string | undefined }[];
  resolvedValue?: string; // We'll fill this once the LLM determines a best value
  resolutionExplanation?: string; // Optional explanation from the LLM
}
...
export class GoogleSheetsComparisonAgent extends PolicySynthAgent {
  ...
  async process(): Promise<void> {
    ...
    const allImportedData: Record<string, JobDescription[]> = {};
    ...
    for (let i = 0; i < this.sheetNames.length; i++) {
      ...
      this.logger.info(
        `Imported ${allImportedData[sn].length} job descriptions from sheet "${sn}".`
      );
    }

    // 2. Collect all unique titleCodes to compare across sheets
    const allTitleCodes = new Set<string>();
    for (const sn of this.sheetNames) {
      allImportedData[sn].forEach((jd) => {
        if (jd.titleCode) allTitleCodes.add(jd.titleCode);
      });
    }
    ...
    for (const titleCode of allTitleCodes) {
      ...
      for (const field of fieldsToCheck) {
        const allValues = new Set<string | undefined>();
        const sheetValuePairs: { sheetName: string; value: string | undefined }[] = [];

        for (const sn of this.sheetNames) {
          const jd = rowDataPerSheet[sn];
          if (!jd) continue;

          const rawVal = (jd as any)[field];
    ...
    for (const diff of differences) {
      diffCount++;
      await this.updateRangedProgress(
        50 + (diffCount / differences.length) * 25,
        `Resolving differences ${diffCount} of ${differences.length}`
      );

      // Try to retrieve the original job description from memory
      // (assuming your memory has some 'jobDescriptions' array).
      const originalJD: JobDescription | undefined = this.memory.jobDescriptions?.find(
        (jd: JobDescription) => jd.titleCode === diff.titleCode
      );

      // If we can't find the original, we can fallback to a minimal prompt or skip.
      if (!originalJD) {
        this.logger.warn(
          `No original job description found in memory for titleCode=${diff.titleCode}. Skipping LLM resolution.`
        );
        continue;
      }

      try {
        const { resolvedValue, explanation } = await this.resolveDifferenceWithLLM(
          originalJD,
          diff
        );
        ...
        this.logger.info(
          `titleCode=${diff.titleCode}, field=${diff.field}, resolvedValue="${resolvedValue}"`
        );
      } catch (err) {
        this.logger.error(
          `Error resolving difference for titleCode=${diff.titleCode}, field=${diff.field}: ${(err as Error).message}`
        );
      }
    }
    ...
    this.logger.info(
      `Differences with LLM resolutions: ${differences.filter((d) => d.resolvedValue).length}`
    );
  ...
  ): Promise<{ resolvedValue: string; explanation: string }> {
...
${diff.values
  .map(
    (pair) =>
      `- From sheet "${pair.sheetName}": ${pair.value ?? "(empty)"}`
  )
  .join("\n")}
...
\`\`\`
    ...
    const results = await this.callModel(
