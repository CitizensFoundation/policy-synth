 ...
 */
export class DeduplicationByKeysAgent<T> extends PolicySynthAgent {
  ...
  async deduplicateItems(
    ...
    hasOrganizationName: boolean = false
  ): Promise<T[]> {
    this.hasOrganizationName = hasOrganizationName;
    this.logger.debug(
      `-----------------------------------> Deduplicating items: ${JSON.stringify(
        items,
        null,
        2
      )}`
    ...
    if (items.length < 2) {
      this.logger.debug(
        `-----------------------------------> No duplicates found, returning items as is`
      );
      return items;
    }
    ...
    const normalizedItems: { item: T; normalizedKey: string }[] = items.map(
      (item) => {
    ...
    for (const group of Object.values(grouped)) {
      if (group.length === 1) {
        ...
        if (this.hasOrganizationName) {
          organizationName = (group[0] as any).organizationName;
        }
        await this.updateRangedProgress(
          undefined,
          `Deduplicating Web Content ${
    ...
    this.logger.debug(
      `-----------------------------------> Deduplicated items: ${JSON.stringify(
        deduplicated,
        null,
        2
      )}`
...
- Always return strict JSON, no extra text.`;
  ...
  private async mergeDuplicatesWithLLM(
    duplicates: T[],
    systemPrompt: string | undefined = undefined
  ): Promise<T | null> {
...
Your JSON Output:`;
    ...
    console.log(
      `DeduplicationAgent: Calling LLM with prompt: ${JSON.stringify(
        messages,
        null,
        2
      )}`
    ...
    if (!response.organizationName || response.organizationName.trim() === "") {
      ...
      const fallback = duplicates.find(
        (d: any) => d.organizationName && d.organizationName.trim()
      ) as any;
      if (fallback) response.organizationName = fallback.organizationName;
    }

    console.log(
      `DeduplicationAgent: LLM response: ${JSON.stringify(response, null, 2)}`
    );
