...
export class DegreeRequirementAnalyzerAgent extends PolicySynthAgent {
    ...
    modelSize = PsAiModelSize.Medium; // Or Large for complex analysis
...
Your JSON output:`;
        try {
            ...
            if (!analysis ||
                ...
                this.logger.error(`LLM response parsing failed or invalid structure for ${licenseType}. Response: ${JSON.stringify(llmResponse)}`);
                this.memory.llmErrors.push(`Analyzer LLM Parsing Error (${licenseType}): Invalid JSON structure`);
                return {
                    error: `LLM analysis failed for ${licenseType}: Invalid response structure.`,
                };
            }
            this.logger.info(`Analysis complete for ${licenseType}: Status=${analysis.degreeRequiredStatus}, Confidence=${analysis.confidenceScore}`);
            await this.updateRangedProgress(100, `Analysis complete for: ${licenseType}`);
            return analysis;
        }
        catch (error) {
            this.logger.error(`Error during LLM analysis for ${licenseType}: ${error.message}`);
            this.memory.llmErrors.push(`Analyzer LLM Error (${licenseType}): ${error.message}`);
            await this.updateRangedProgress(100, `Analysis failed for: ${licenseType}`);
            return {
...
//# sourceMappingURL=requirementAnalyzer.js.map