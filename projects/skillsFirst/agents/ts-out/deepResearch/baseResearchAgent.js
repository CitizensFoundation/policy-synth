...
export class BaseDeepResearchAgent extends PolicySynthAgent {
    useDebugCache = false;
    ...
    convertFromCamelCaseToReadable(dataType) {
    ...
    dataType = undefined, statusPrefix = "") {
        ...
        try {
            ...
            if (dataType) {
                await this.updateRangedProgress(this.startProgress + 0.3 * totalProgressRange, `${statusPrefix}Ranking Search Queries (${this.convertFromCamelCaseToReadable(dataType)})`);
            }
            else {
                await this.updateRangedProgress(this.startProgress + 0.3 * totalProgressRange, `${statusPrefix}Ranking Search Queries`);
            }
            ...
            const subAgentStartProgress = this.startProgress + subAgentStartPercentage * totalProgressRange;
            const subAgentEndProgress = this.startProgress + subAgentEndPercentage * totalProgressRange;
            const searchQueriesRanker = new SearchQueriesRanker(this.agent, this.memory, undefined, subAgentStartProgress, subAgentEndProgress);
            ...
            await this.updateRangedProgress(this.startProgress + 0.4 * totalProgressRange, `${statusPrefix}Searching the Web`);
            ...
            if (dataType) {
                await this.updateRangedProgress(this.startProgress + 0.5 * totalProgressRange, `${statusPrefix}Ranking Search Results (${this.convertFromCamelCaseToReadable(dataType)})`);
            }
            else {
                await this.updateRangedProgress(this.startProgress + 0.5 * totalProgressRange, `${statusPrefix}Ranking Search Results`);
            }
            const resultsRankerStartProgress = this.startProgress + 0.5 * totalProgressRange;
            const resultsRankerEndProgress = this.startProgress + 0.65 * totalProgressRange;
            const searchResultsRanker = new SearchResultsRanker(this.agent, this.memory, undefined, resultsRankerStartProgress, resultsRankerEndProgress, this.useSmallModelForSearchResultsRanking);
            ...
            const scannerStartProgress = this.startProgress + 0.65 * totalProgressRange;
            const scannerEndProgress = this.startProgress + 0.8 * totalProgressRange;
            const webPageResearch = new WebPageScanner(this.agent, this.memory, scannerStartProgress, scannerEndProgress, this.searchInstructions, this.scanningSystemPrompt);
            let webScanResults = await webPageResearch.scan(searchResultsToScan.map((i) => i.url), this.scanType, this.urlToCrawl);
            this.logger.info("Website Scanning Completed", true);
            console.log(`webScanResults before dedup: ${webScanResults.length}`);
            // First flatten the array
            webScanResults = webScanResults.flat();
            // Then filter out any empty or null values
            webScanResults = webScanResults.filter((item) => item && Object.keys(item).length > 0);
            if (this.attributeNameToUseForDedup === "organizationName") {
                webScanResults = webScanResults.filter((item) => {
                    if (!item.organizationName || item.organizationName.trim() === "") {
                        this.logger.info(`Removing item with no name: ${JSON.stringify(item)}`);
                        return false;
                    }
                    return true;
                });
            ...
            if (this.findOrganizationUrlsAndLogos && this.attributeNameToUseForDedup) {
                const dedupAgent = new DeduplicationByKeysAgent(this.agent, this.memory, this.startProgress, this.endProgress);
                await this.updateRangedProgress(this.startProgress + 0.8 * totalProgressRange, `${statusPrefix}Deduplicating Web Content`);
                webScanResults = await dedupAgent.deduplicateItems(webScanResults, (item) => {
                    const val = item[this.attributeNameToUseForDedup];
                    // Ensure we always return a string key:
                    return typeof val === "string"
                        ? val.toLowerCase().trim()
                        : JSON.stringify(val);
                }, undefined, this.attributeNameToUseForDedup === "organizationName");
                console.log(`webScanResults after LLM dedup: ${webScanResults.length}`);
                console.log(`webScan: (${webScanResults.length}) ${JSON.stringify(webScanResults, null, 2)}`);
            }
            if (!this.disableRanking) {
                const contentRankerStartProgress = this.startProgress + 0.8 * totalProgressRange;
                const contentRankerEndProgress = this.endProgress;
                const webContentRanker = new DeepResearchWebContentRanker(this.agent, this.memory, undefined, contentRankerStartProgress, contentRankerEndProgress);
                await this.updateRangedProgress(this.startProgress + 0.8 * totalProgressRange, `${statusPrefix}Ranking Web Content`);
                webScanResults = await webContentRanker.rankWebContent(webScanResults, this.rankingInstructions);
                let topWebScanResults = webScanResults.slice(0, maxTopContentResultsToUse);
                if (this.useDebugCache) {
                    try {
                ...
                await this.updateRangedProgress(this.endProgress, `${statusPrefix}Web Research Completed`);
