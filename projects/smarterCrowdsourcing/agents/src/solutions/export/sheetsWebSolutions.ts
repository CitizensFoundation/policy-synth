...
export class SolutionsFromSearchSheetsExportAgent extends PolicySynthAgent {
  ...
  async process(): Promise<void> {
    ...
    if (problemStatementSolutions && problemStatementSolutions.length > 0) {
      await this.exportSolutions(
        "ProblemStatement_SolutionsFromSearch",
    ...
    for (let i = 0; i < subProblems.length; i++) {
      ...
      if (subProblem.entities && subProblem.entities.length > 0) {
        for (let j = 0; j < subProblem.entities.length; j++) {
          ...
          const entitySolutions = (entity as any).solutionsFromSearch; // Ensure entity has solutionsFromSearch

          if (entitySolutions && entitySolutions.length > 0) {
            entitySolutions.forEach((solution: PsSolution) => {
              combinedSolutions.push({
      ...
      if (combinedSolutions.length > 0) {
        ...
        await this.exportSolutions(
          sheetName,
          combinedSolutions.map(cs => cs.solution),
          `SubProblem ${i + 1}: ${subProblem.title}`,
          combinedSolutions.map(cs => cs.source).join(", ")
        );
      } else {
   ...
   */
  private prepareSolutionsData(solutions: PsSolution[], sourceDescription: string): string[][] {
    const headers = [
      "Title",
      "Description",
    ...
    const rows = solutions.map((solution) => {
      return[
        ...
        solution.imageUrl || "",
        solution.fromUrl || "",
        solution.contacts ? solution.contacts.join("\n") : ""
        // Add other fields as needed
      ];
    });
...
/**
 * Interface to represent a combined solution with its source.
 */
interface CombinedSolution {
  source: string; // e.g., "SubProblem" or "Entity: EntityName"
  solution: PsSolution;
}
