...
export class RankWebSolutionsAgent extends BaseSmarterCrowdsourcingPairwiseAgent {
  async voteOnPromptPair(
    ...
    const messages = [
      this.createSystemMessage(
        `You're an expert in evaluating and ranking solutions to problems.
  ...
  async processSubProblem(subProblemIndex: number) {
    ...
    if (
      !this.memory.subProblems[subProblemIndex].solutionsFromSearch![0]
        .eloRating
    ) {
      this.setupRankingPrompts(
        subProblemIndex,
        this.memory.subProblems[subProblemIndex].solutionsFromSearch!,
        this.memory.subProblems[subProblemIndex].solutionsFromSearch!.length *
          10
      );
    ...
    try {
      if (!this.memory.problemStatement.solutionsFromSearch![0].eloRating) {
        this.setupRankingPrompts(
          ...
          this.memory.problemStatement.solutionsFromSearch!.length * 10
        );
