...
export class EvolvePopulationAgent extends CreateInitialSolutionsAgent {
    ...
    return JSON.stringify(
      {
        ...
        mainBenefitOfSolution:
          solution.mainBenefitOfSolution ||
          solution.mainBenefitOfSolution,
        mainObstacleToSolutionAdoption:
  ...
  async getNewSolutions(
    alreadyCreatedSolutions: PsSolution[],
    subProblemIndex: number
  ) {
    ...
    const solutionsForInspiration = await this.getRandomSolutions(
      subProblemIndex,
      undefined
    );

    this.logger.debug(
      `Evolution Text contexts: ${JSON.stringify(
  ...
  addUniqueAboveAverageSolutionAsElite(
    ...
    const groups = new Map<number, Array<PsSolution>>();
  ...
  pruneTopicClusters(
    ...
    const prunedSolutionSet = new Set<PsSolution>();
  ...
  async evolveSubProblem(subProblemIndex: number) {
    ...
    const newPopulation: PsSolution[] = [];
