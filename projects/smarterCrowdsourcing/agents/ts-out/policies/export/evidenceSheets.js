...
export class PoliciesSheetsExportAgent extends PolicySynthAgent {
    ...
    async process() {
        ...
        for (let i = 0; i < subProblems.length; i++) {
            ...
            for (let j = 0; j < policiesPopulation.length; j++) {
                ...
                const sheetName = this.sanitizeSheetName(`Solution_${policy.title.substring(0, 50)}`);
                ...
                console.log(`Exported evidence for policy: ${policy.title} to sheet: ${sheetName}`);
                // Update progress
                const progressPercentage = ((j + 1) / policiesPopulation.length) * 100;
                await this.updateRangedProgress(progressPercentage, `Processed policy ${j + 1}/${policiesPopulation.length} in subproblem ${i + 1}/${subProblems.length}`);
            }
        }
     ...
     */
    preparePolicyEvidenceData(policy) {
        const headers = [
            "Evidence URL",
            "Summary",
        ...
        if (!policy.webEvidence || policy.webEvidence.length === 0) {
            ...
            policy.webEvidence.forEach((evidence) => {
                if (evidence.contentPublishingYear &&
                    ...
                    console.warn(`Evidence not included in export due to low scores or missing data: ${evidence.url}`);
                }
            });
        }
...
//# sourceMappingURL=evidenceSheets.js.map