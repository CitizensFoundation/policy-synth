...
export class CreateInitialSolutionsAgent extends SolutionsEvolutionSmarterCrowdsourcingAgent {
    renderCreateSystemMessage() {
        return this.createSystemMessage(`As an expert, you are tasked with creating innovative solutions for sub problems, considering the affected entities based on the <UserSolutionsToBaseYourSolutionsOn> provided by the user.
    ...
    async createSolutions(subProblemIndex, solutionsForInspiration, alreadyCreatedSolutions = undefined, stageName = "create-seed-solutions") {
        if (DISABLE_LLM_FOR_DEBUG) {
            ...
            let results = await this.callModel(PsAiModelType.Text, PsAiModelSize.Medium, await this.renderCreatePrompt(subProblemIndex, solutionsForInspiration, alreadyCreatedSolutions), true, false, 860);
            return results;
        }
    }
    ...
    getRandomSolutions(subProblemIndex, alreadyCreatedSolutions) {
        ...
        this.logger.debug(`Lengths: Entities: ${entitiesSolutions.length}, SubProblem: ${subProblemSolutions.length}, ProblemStatement: ${problemStatementSolutions.length}`);
        ...
        if (randomValue < 0.05) {
            ...
            if (subProblemSolutions.length > 8 && Math.random() < 0.5) {
                selectedSolutions = this.getRandomItemsFromArray(getFilteredSolutions(entitiesSolutions, 1000), numberOfSolutionComponents);
                ...
                selectedSolutions = this.getRandomItemsFromArray(entitiesSolutions, numberOfSolutionComponents);
            }
            this.logger.debug(`Selected solutions from entities: ${JSON.stringify(selectedSolutions.map((s) => `${s.title}: ${s.description}`), null, 2)}`);
        }
        ...
        if (selectedSolutions.length < numberOfSolutionComponents) {
            this.logger.warn(`Not enough solutions found, only ${selectedSolutions.length} selected`);
            const missingCount = numberOfSolutionComponents - selectedSolutions.length;
            const missingSolutions = this.getRandomItemsFromArray(problemStatementSolutions, missingCount);
            selectedSolutions = selectedSolutions.concat(missingSolutions);
            this.logger.debug(`Missing solutions added: ${JSON.stringify(missingSolutions.map((s) => `${s.title}: ${s.description}`), null, 2)}`);
        }
