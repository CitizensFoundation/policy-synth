...
export class setOgInfoForTopEvidenceProcessor extends BaseProcessor {
    ...
    async setEvidenceType(policy, subProblemIndex) {
        ...
        while (true) {
            try {
                const results = await this.evidenceWebPageVectorStore.getWebPagesForProcessing(this.memory.groupId, subProblemIndex, undefined, policy.title, limit, offset);
