...
for (
  ...
  for (const solution of solutions) {
    ...
    pros = pros.map((pro: any) =>
      typeof pro === "string" ? pro : pro.description
    );
    cons = cons.map((con: any) =>
      typeof con === "string" ? con : con.description
    );
      ...
      <div class="card">
        ...
        ${
          pros.length > 0
            ? `<h3>Pros:</h3>
              <ul>
              ${pros.map((pro: string) => `<li>${pro}</li>`).join("")}
              </ul>`
            : ""
        }
        ...
        ${
          cons.length > 0
            ? `<h3>Cons:</h3>
              <ul>
              ${cons.map((con: string) => `<li>${con}</li>`).join("")}
              </ul>`
            : ""
        }
      </div>
