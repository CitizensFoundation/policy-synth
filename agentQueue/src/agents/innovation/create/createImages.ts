...
export class CreateSolutionImagesProcessor extends BaseProcessor {
        ...
        if (error.message && error.message.indexOf("400") > -1) {
          if (retryCount > 3) {
            imagePrompt = (await this.callLLM(
              ...
              await this.renderCreatePrompt(subProblemIndex, solution!, "8. Make it very simple and colorful with no complicated ideas or details."),
              false
            )) as string;
  ...
  async renderCreatePrompt(subProblemIndex: number, solution: IEngineSolution, injectText?: string) {
