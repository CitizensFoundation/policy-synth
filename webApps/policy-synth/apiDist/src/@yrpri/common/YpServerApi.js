...
export class YpServerApi extends YpServerApiBase {
    ...
    getTranslatedRegistrationQuestions(groupId, targetLanguage) {
        return this.fetchWrapper(this.baseUrlPath + `/groups/${groupId}/translatedRegistrationQuestions?targetLanguage=${targetLanguage}`);
    }
    ...
    getSurveyTranslations(post, language) {
        return this.fetchWrapper(this.baseUrlPath + `/posts/${post.id}/translatedSurvey?targetLanguage=${language}&groupId=${post.Group.id}`);
    }
