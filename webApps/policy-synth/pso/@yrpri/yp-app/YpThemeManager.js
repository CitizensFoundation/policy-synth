...
export class YpThemeManager {
    ...
    _setupOverrideTheme(primary, accent, background) {
    ...
    _onlyGetStylesFromTheme(properties) {
        const filterdProperties = {};
        for (const property in properties) {
            if (Object.prototype.hasOwnProperty.call(properties, property)) {
                if (property !== 'name' && property !== 'disabled') {
                    filterdProperties[property] = properties[property];
                }
            }
        }
    ...
    setTheme(number, configuration = undefined) {
        if (configuration &&
            ...
            if (configuration.themeOverrideBackgroundColor &&
                configuration.themeOverrideBackgroundColor.length === 6) {
                configuration.themeOverrideBackgroundColor =
                    '#' + configuration.themeOverrideBackgroundColor;
            }
            this.updateStyles(this._setupOverrideTheme(configuration.themeOverrideColorPrimary, configuration.themeOverrideColorAccent, configuration.themeOverrideBackgroundColor));
        }
        else if (number !== undefined) {
            if (this.themes[number]) {
                this.updateStyles(this._onlyGetStylesFromTheme(this.themes[number]));
            }
        }
